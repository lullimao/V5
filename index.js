...
const questions = [
    {
        ...
        validate: function (input) {
            ...
            if (!input) {
                done('Please input your auth key!');
                return false;
            }
            let authParse;
            try {
            ...
            if (typeof authParse != "object") {
                done("Please input your auth key as Obj!");
                return false;
            }
...
const asciiText = figlet.textSync("lol nah", {
    font: 'Graffiti',
    ...
    setInterval(async function iStumble() {
        try {
            ...
            if (typeof data == "string" && data.includes("BANNED")) {
                console.error(color("Account Banned!", "redBright"));
            } else if (typeof data == "object") {
                const date = new Date();
                let { Username, Crowns, SkillRating } = data.User;
                const print = `[${color(date.getHours())}:${date.getMinutes()}] ` + [color('lol nah'), color(Username), color(Crowns, "red"), color(SkillRating, "yellowBright")].join(", ");
                console.log(print);
            }
        } catch (error) {}
    }, Number(interval));
